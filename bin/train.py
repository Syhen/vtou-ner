...
valid_idx = idx[int(len(idx) * 0.7): int(len(idx) * 0.8)]
...
model = BiLSTMCRF(SEQ_LEN, len(vocab) + 1, tag_size)
...
model_trainer.fit(x_train, y_train, epochs=EPOCHS, batch_size=32, shuffle=False, use_gpu=False)
test_pred = model_trainer.predict(x_test, batch_size=16)
# print(test_pred)
print(model_trainer.evaluate(y_test, test_pred))
